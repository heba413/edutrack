...
const s_signup = async (schoolData) => {
  const { name, phone, email, password } = schoolData;
  ...
  return { token, email: newSchool.email, name: newSchool.name, phone: newSchool.phone };
};
...
const s_login = async (email, password) => {
  ...
  return { token, phone: school.phone, email: school.email, name: school.name };
};
